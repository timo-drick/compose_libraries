...
float sdLegs(vec3 p) {
    ...
    p.y += 1.65;
    p.x -= 0.5;
    return sdCappedCylinder(p, 0.5, 0.2);
}

Material GetDist(vec3 p) {
    Material background = Material(p.z + 2.0, vec3(0.3));
    p.yz *= Rot(-PI * 0.4);
    float angle = abs(PI * 2.0 * (iTime - .5)) + PI / 4.0 * 1.5;
    //float angle = PI;
    p.xz *= Rot(angle);

    Material eye = Material(sdEyes(p), vec3(0.05));
    float bodyD = min(sdHead(p), sdTorso(p));
    bodyD = opSmoothUnion(bodyD, sdLegs(p), 0.03);
    Material greenBody = Material(bodyD, fg.rgb);
...
vec4 renderScene(vec2 uv) {
    ...
    if(d<MAX_DIST) {
        ...
        float spe = pow(max(0.0, dot(rd, reflect(lightToPoint, n))), 20.0);
