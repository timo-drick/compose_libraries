...
Material GetDist(vec3 p) {
    ...
    Material x = Material(arrow(p, start, vec3(1,0,0)*1.5), vec3(1,0,0));
    Material y = Material(arrow(p, start, vec3(0,1,0)*1.5), vec3(0,1,0));
    Material z = Material(arrow(p, start, vec3(0,0,1)*1.5), vec3(0,0,1));
    Material m = mixMaterial(x, y);
    m = mixMaterial(m, z);
    //m = mixMaterial(m, Material(., vec3(0,1,0)));
    return m;
}
